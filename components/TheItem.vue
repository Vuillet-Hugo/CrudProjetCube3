...
const handleCancel = (): void => {
    ...
    formInput!.value = props.item.nom
}
...
<template>
    ...
    >
        <div class="w-full text-sm text-slate-600">
            <div class="item w-full flex justify-around">
                <span>{{ item.nom }}</span><span>{{ item.type }}</span><span>{{ item.niveau }}</span>
